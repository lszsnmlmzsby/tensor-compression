...
class TestTensorEditJsonlDataset(unittest.TestCase):
    def test_leaves_clean_chinese_prompt_unchanged(self) -> None:
        clean_prompt = "传感器链路处于标准模式，Vx读数没有系统误差。"
        record = {
            "prompt": clean_prompt,
        ...
        self.assertEqual(sample["raw_prompt"], clean_prompt)
    ...
    def test_repairs_utf8_prompt_decoded_as_gbk(self) -> None:
        clean_prompt = "传感器链路处于标准模式，Vx读数没有系统误差。请将该512x512网格视为真实状态。"
        mojibake_prompt = clean_prompt.encode("utf-8").decode("gbk")
        record = {
            "prompt": mojibake_prompt,
            "tensor": [[0.0, 1.0], [2.0, 3.0]],
            "label": [[0.0, 1.0], [2.0, 3.0]],
