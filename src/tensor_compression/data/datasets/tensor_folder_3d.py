...
@DATASET_REGISTRY.register("tensor_folder_3d")
class TensorFolder3DDataset(BaseTensorDataset):
    ...
    def __init__(self, config: dict, split: str) -> None:
        ...
        if not self.samples and not self.allow_empty:
            ...
            raise FileNotFoundError(
                f"No 3D tensor files found for split={split!r} under: {joined_roots}"
            )
    ...
    def _build_hdf5_samples(self, path: Path) -> list[dict]:
        with h5py.File(path, "r") as handle:
            ...
            if dataset_path is None:
                ...
                raise ValueError(
                    "No suitable numeric 3D/4D/5D/6D dataset found in "
    ...
    def _load_tensor(self, sample: dict) -> torch.Tensor:
        ...
        if suffix == ".npy":
            ...
            raise ValueError(
                f"Unsupported 3D tensor file: {path}. "
                "Use .npy, .npz, .h5, or .hdf5 for 3D inputs."
            )
        tensor = self._to_tensor(array)
        tensor = self._ensure_cdhw(tensor)
    ...
    def _is_supported_hdf5_dataset(self, dataset: h5py.Dataset) -> bool:
        if not np.issubdtype(dataset.dtype, np.number):
            return False
        return dataset.ndim in (3, 4, 5, 6)
    ...
    def _resolve_hdf5_index_mode(self, shape: tuple[int, ...]) -> str:
        ...
        if ndim <= 3:
            return "file"
        if ndim == 4:
            if shape[0] in channel_hints or shape[-1] in channel_hints:
                return "file"
            return "sample"
        if ndim >= 5:
            return "sample"
    ...
    def _resolve_hdf5_sample_axes(
        ...
        if not self._is_valid_3d_sample_shape(shape, axes):
            raise ValueError(
                "Configured HDF5 sample axes do not leave a valid 3D sample shape. "
                f"Got dataset shape={shape}, sample_axes={axes}. "
                "Adjust `hdf5_sample_axes` (for example [0, 1] for [N, T, D, H, W]) "
    ...
    def _augment_sample_axes(self, shape: tuple[int, ...], axes: list[int]) -> list[int]:
        ...
        while not self._is_valid_3d_sample_shape(shape, resolved_axes):
    ...
    def _is_valid_3d_sample_shape(self, shape: tuple[int, ...], sample_axes: list[int]) -> bool:
        ...
        if len(remaining) != 4:
            return False
        channel_hints = {self.channels, 1, 3, 4}
        return remaining[0] in channel_hints or remaining[-1] in channel_hints
    ...
    def _ensure_cdhw(self, tensor: torch.Tensor) -> torch.Tensor:
        channel_hints = {self.channels, 1, 3, 4}
        if tensor.ndim == 3:
            ...
            if tensor.shape[0] in channel_hints:
                ...
                raise ValueError(
                    "Cannot infer 3D tensor channel order from shape "
                    f"{tuple(tensor.shape)}. If this came from an HDF5 sequence like "
                    "[N, T, D, H, W], set `hdf5_sample_axes: [0, 1]` so sequence axes "
                    "are expanded into samples instead of being treated as channels."
                )
        else:
            raise ValueError(
                f"Expected 3D tensor with 3 or 4 dims, got shape {tuple(tensor.shape)}."
            )
    ...
    def _ensure_channel_count(self, tensor: torch.Tensor) -> torch.Tensor:
        ...
        if tensor.shape[0] == 1 and self.channels > 1:
            repeats = [self.channels] + [1] * (tensor.ndim - 1)
    ...
    def _resize_if_needed(self, tensor: torch.Tensor) -> torch.Tensor:
        ...
        if self.strict_size:
            raise ValueError(
                f"Expected input size {(target_d, target_h, target_w)}, got {(depth, height, width)}."
            )
        tensor = tensor.unsqueeze(0)
        kwargs = {
            "input": tensor,
        ...
        if self.resize_mode in {"trilinear"}:
