...
@MODEL_REGISTRY.register("conv_token_autoencoder_2d")
class ConvTokenAutoencoder2D(BaseCompressionModel):
    def __init__(self, config: dict) -> None:
        ...
        if input_size[0] != latent_grid[0] * down_factor or input_size[1] != latent_grid[1] * down_factor:
            raise ValueError(
        ...
        for idx, mult in enumerate(reversed_multipliers):
            out_channels = base_channels * reversed_multipliers[idx + 1] if idx + 1 < len(reversed_multipliers) else base_channels
        ...
        decoder_layers.append(nn.Conv2d(current_channels, self.out_channels, kernel_size=3, padding=1))
